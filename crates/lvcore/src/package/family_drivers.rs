...
use super::capabilities::{
    hourei_capabilities, lved_capabilities, multiview_capabilities, ssed_aux_index_search_modes,
    ssed_search_modes, ssed_sidecar_search_modes, ssed_sizk_search_modes, standard_search_modes,
};
use super::drivers::{
...
impl SsedDriver {
    fn open_with_catalog(
        ...
        if search_modes.is_empty() {
            let aux_modes = ssed_aux_index_search_modes(&package_root)?;
            if !aux_modes.is_empty() {
                extend_unique_search_modes(&mut search_modes, aux_modes);
                extend_unique_capabilities(
                    &mut capabilities,
                    vec![super::Capability::NativeSearch],
                );
        ...
        if search_modes.is_empty() {
            let sizk_modes = ssed_sizk_search_modes(&package_root)?;
            if !sizk_modes.is_empty() {
                extend_unique_search_modes(&mut search_modes, sizk_modes);
                extend_unique_capabilities(
                    &mut capabilities,
                    vec![
                        super::Capability::NativeSearch,
                        super::Capability::FullTextSearch,
                    ],
                );
            }
        }
    ...
    fn detect(&self, root: &Path) -> Result<Option<DetectedPackage>> {
        ...
        if let Some(store) = LvedSqliteStore::discover(root)? {
            if detect_ssed_package(package_root)?.is_some()
                && store.key_file.is_none()
                && store.android_info.is_none()
            {
                return Ok(None);
            }
            ...
            let title = store.cheap_title_hint();
