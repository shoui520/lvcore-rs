...
const SSED_TITLE_LABEL_EARLY_HIT_LOOKAHEAD_MAX_CURSOR_OFFSET: usize = 8;
const SSED_TITLE_LABEL_EARLY_HIT_LOOKAHEAD_MAX_INDEX_BLOCKS: u32 = 768;
...
impl ReaderBookPackage {
    ...
    fn search_ssed_partial_nonprefix_page_inner_with_seen_targets(
        ...
        let deferred_next_cursor = if defer_next_page_proof && !page.hits.is_empty() {
            matched_offset_physical_start.map(|cursor| {
                if skip_prefix_rows {
                    encode_ssed_partial_unverified_nonprefix_cursor(Some(cursor))
                } else {
                    encode_ssed_partial_unverified_nonprefix_physical_offset_cursor(
                        cursor,
                        offset.saturating_add(page.hits.len()).to_string(),
                        skip_prefix_rows,
                    )
                }
            })
    ...
    fn search_ssed_initial_partial_nonprefix_page(
        &self,
        query: &SearchQuery,
        needle: &str,
        skip_prefix_rows: bool,
        ...
        Ok(self
            .search_ssed_partial_nonprefix_page_inner_with_seen_targets(
                query,
                needle,
                None,
                skip_prefix_rows,
                HashSet::new(),
                self.ssed_partial_nonprefix_fill_should_defer(),
            )?
            .page)
    }
    ...
    fn search_ssed_title_label_fallback_page_inner(
        ...
        let early_hit_page_max_rows = self.ssed_title_label_fallback_early_hit_page_max_rows(query);
        let fallback_diagnostics = self.scan_ssed_ordered_index_rows_with_filters(
            ...
            |row| {
                ...
                let row_budget = if hits.is_empty() {
                    empty_page_max_rows
                } else if page_full_cursor.is_some_and(|cursor| {
                    cursor <= SSED_TITLE_LABEL_EARLY_HIT_LOOKAHEAD_MAX_CURSOR_OFFSET
                }) {
                    early_hit_page_max_rows
                } else {
                    hit_page_max_rows
                };
    ...
    fn ssed_title_label_fallback_hit_page_max_rows(&self, query: &SearchQuery) -> usize {
        let Some(index_block_count) = self.ssed_title_label_fallback_index_block_count_up_to(
    ...
    fn ssed_title_label_fallback_early_hit_page_max_rows(&self, query: &SearchQuery) -> usize {
        let Some(index_block_count) = self.ssed_title_label_fallback_index_block_count_up_to(
            SSED_TITLE_LABEL_EARLY_HIT_LOOKAHEAD_MAX_INDEX_BLOCKS,
        ) else {
            return SSED_TITLE_LABEL_SEARCH_FALLBACK_MAX_ROWS;
        };
        ssed_title_label_fallback_early_hit_page_max_rows_for_index_blocks(query, index_block_count)
    }

    fn ssed_title_label_fallback_index_block_count_up_to(
        &self,
        ...
        for component in catalog.components_by_role(SsedComponentRole::Index) {
            ...
            if index_block_count > max_index_blocks {
                return None;
            }
        }
        Some(index_block_count)
    }
    ...
    fn ssed_fulltext_direct_body_scan_page(
        ...
        'components: for component in catalog.components_by_role(SsedComponentRole::Honmon) {
            ...
            if let Some(cursor) = start_cursor
                && component.filename.as_str() < cursor.component.as_str()
            {
                continue;
            }
            let path = match self.resolve_readable_ssed_component_path(component) {
            ...
            let mut scan_offset = if let Some(cursor) = start_cursor
                && component.filename == cursor.component
            {
                ssed_fulltext_scan_start_after_body_cursor(
                    &mut reader,
                    cursor.offset,
                    expanded_size,
                )?
            ...
            while scan_offset < expanded_size {
                ...
                let mut candidate_search_offset = search_start;
                let mut window_has_byte_candidate = false;
                let mut window_has_decoded_candidate = false;
                while candidate_search_offset < data.len() {
                    let Some(candidate_offset) = ssed_fulltext_first_byte_candidate_offset(
                        &data,
                        byte_candidates,
                        candidate_search_offset,
                    ) else {
                        break;
                    };
                    if !window_has_byte_candidate {
                        byte_candidate_windows = byte_candidate_windows.saturating_add(1);
                        window_has_byte_candidate = true;
                    }
                    ...
                    .unwrap_or(data.len());
                    let next_candidate_search_offset =
                        entry_end.max(candidate_offset.saturating_add(1));
                    if !seen_entries.insert((component.filename.clone(), absolute_entry_offset)) {
                        candidate_search_offset = next_candidate_search_offset;
                        continue;
                    }
        ...
        let (scan_diagnostic_code, scan_diagnostic_message) = if start_cursor.is_some() {
            (
                "ssed_fulltext_body_cursor_scan",
                "SSED full-text search resumed native HONMON body scanning from a physical cursor",
            )
        ...
        diagnostics.push(
            Diagnostic::info(scan_diagnostic_code, scan_diagnostic_message)
        ...
        let next_cursor = if query.limit > 0 && hits.len() >= query.limit {
            next_cursor_after_returned
                .as_ref()
                .map(encode_ssed_fulltext_body_physical_cursor)
        } else {
...
struct SsedDirectFulltextRequest<'a> {
    ...
    start_cursor: Option<&'a SsedFulltextBodyCursor>,
}

fn ssed_fulltext_scan_start_after_body_cursor(
    reader: &mut SsedDataFile,
    cursor_offset: u64,
    expanded_size: usize,
) -> Result<usize> {
    let cursor_offset = usize::try_from(cursor_offset)
        .map_err(|_| Error::Driver("SSED body cursor offset does not fit in usize".to_owned()))?;
    if cursor_offset >= expanded_size {
        return Ok(expanded_size);
    }

    let marker_len = SSED_ENTRY_MARKER.len();
    let probe_size = expanded_size
        .saturating_sub(cursor_offset)
        .min(marker_len.saturating_add(2));
    let probe = reader.read_range(cursor_offset, probe_size)?;
    let current_marker_offset = if probe.len() >= marker_len.saturating_add(2)
        && probe[..2] == [0x1f, 0x02]
        && probe[2..2 + marker_len] == SSED_ENTRY_MARKER
    {
        cursor_offset.saturating_add(2)
    } else if probe.len() >= marker_len && probe[..marker_len] == SSED_ENTRY_MARKER {
        cursor_offset
    } else {
        cursor_offset
    };

    let marker_overlap = marker_len.saturating_sub(1);
    let mut search_offset = current_marker_offset.saturating_add(marker_len);
    while search_offset < expanded_size {
        let read_start = search_offset.saturating_sub(marker_overlap);
        let read_size = expanded_size
            .saturating_sub(read_start)
            .min(SSED_FULLTEXT_SCAN_WINDOW_BYTES.saturating_add(marker_overlap));
        let data = reader.read_range(read_start, read_size)?;
        if data.is_empty() {
            break;
        }
        let marker_search_start = search_offset.saturating_sub(read_start);
        if let Some(marker) = ssed_fulltext_next_entry_marker_offset(&data, marker_search_start) {
            let absolute_marker = read_start.saturating_add(marker);
            return Ok(if marker >= 2 && data[marker - 2..marker] == [0x1f, 0x02] {
                absolute_marker.saturating_sub(2)
            } else {
                absolute_marker
            });
        }
        if search_offset + SSED_FULLTEXT_SCAN_WINDOW_BYTES >= expanded_size {
            break;
        }
        search_offset += SSED_FULLTEXT_SCAN_WINDOW_BYTES;
    }
...
const SSED_PARTIAL_NONPREFIX_UNVERIFIED_PHYSICAL_OFFSET_CURSOR_PREFIX: &str =
    "ssed-partial-nonprefix-unverified-physical-offset:";
...
const SSED_PARTIAL_NONPREFIX_NOSKIP_UNVERIFIED_PHYSICAL_OFFSET_CURSOR_PREFIX: &str =
    "ssed-partial-nonprefix-noskip-unverified-physical-offset:";
...
fn decode_ssed_partial_nonprefix_cursor(
    ...
    if let Some(value) =
        cursor.strip_prefix(SSED_PARTIAL_NONPREFIX_UNVERIFIED_PHYSICAL_OFFSET_CURSOR_PREFIX)
    {
        return decode_ssed_partial_nonprefix_physical_offset_cursor(value, true);
    }
    if let Some(value) =
        cursor.strip_prefix(SSED_PARTIAL_NONPREFIX_NOSKIP_UNVERIFIED_PHYSICAL_OFFSET_CURSOR_PREFIX)
    {
...
fn encode_ssed_partial_unverified_nonprefix_physical_offset_cursor(
    cursor: SsedPartialIndexScanCursor,
    offset: String,
    skip_prefix_rows: bool,
) -> String {
    let prefix = if skip_prefix_rows {
        SSED_PARTIAL_NONPREFIX_UNVERIFIED_PHYSICAL_OFFSET_CURSOR_PREFIX
    } else {
        SSED_PARTIAL_NONPREFIX_NOSKIP_UNVERIFIED_PHYSICAL_OFFSET_CURSOR_PREFIX
    };
    format!(
        "{prefix}{}:{}:{offset}",
        cursor.component_index, cursor.page_index
...
fn ssed_title_label_fallback_early_hit_page_max_rows_for_index_blocks(
    query: &SearchQuery,
    index_block_count: u32,
) -> usize {
    if ssed_title_label_tiny_index_hit_page_lookahead_is_bounded(query, index_block_count)
        || ssed_title_label_early_hit_page_lookahead_is_bounded(query, index_block_count)
    {
        SSED_TITLE_LABEL_TINY_INDEX_SEARCH_FALLBACK_MAX_ROWS
    } else {
        SSED_TITLE_LABEL_SEARCH_FALLBACK_MAX_ROWS
...
fn ssed_title_label_early_hit_page_lookahead_is_bounded(
    query: &SearchQuery,
    index_block_count: u32,
) -> bool {
    ssed_title_label_tiny_index_hit_page_query_is_bounded(query)
        && index_block_count > SSED_TITLE_LABEL_TINY_INDEX_MAX_INDEX_BLOCKS
        && index_block_count <= SSED_TITLE_LABEL_EARLY_HIT_LOOKAHEAD_MAX_INDEX_BLOCKS
}
...
fn ssed_partial_prefix_empty_initial_page_should_fall_through(
    query: &SearchQuery,
    page: &SearchPage,
) -> bool {
    query.cursor.is_none() && page.hits.is_empty()
}
...
mod tests {
    ...
    use super::{
        ...
        ssed_title_label_fallback_early_hit_page_max_rows_for_index_blocks,
    ...
    fn title_label_extended_hit_page_lookahead_is_tiny_index_only() {
        ...
        assert_eq!(
            ssed_title_label_fallback_early_hit_page_max_rows_for_index_blocks(
                &exact_query,
                SSED_TITLE_LABEL_EARLY_HIT_LOOKAHEAD_MAX_INDEX_BLOCKS + 1,
            ),
            SSED_TITLE_LABEL_SEARCH_FALLBACK_MAX_ROWS
        );
        assert_eq!(
    ...
    #[test]
    fn empty_initial_partial_prefix_pages_fall_through() {
        let query = current_book_query(SearchMode::Partial, "1計", 1);
        let empty_prefix_page = SearchPage {
            hits: Vec::new(),
            next_cursor: Some("ssed-prefiltered-index:6:90".to_owned()),
            result_sequence: None,
            diagnostics: Vec::new(),
        };
        assert!(ssed_partial_prefix_empty_initial_page_should_fall_through(
            &query,
            &empty_prefix_page
        ));

        let mut cursor_query = query.clone();
        cursor_query.cursor = Some("ssed-partial-prefix:ssed-prefiltered-index:6:90".to_owned());
        assert!(!ssed_partial_prefix_empty_initial_page_should_fall_through(
            &cursor_query,
            &empty_prefix_page
        ));
    ...
    fn partial_nonprefix_cursors_preserve_prefix_skip_state() {
        ...
        let unverified_physical_offset =
            encode_ssed_partial_unverified_nonprefix_physical_offset_cursor(
                SsedPartialIndexScanCursor {
                    component_index: 8,
                    page_index: 121,
        ...
        assert_eq!(
            unverified_physical_offset,
            "ssed-partial-nonprefix-noskip-unverified-physical-offset:8:121:1"
        );
        assert_eq!(
            decode_ssed_partial_nonprefix_cursor(Some(&unverified_physical_offset)),
            Some(SsedPartialNonprefixCursor::MatchedPhysicalOffset {
                cursor: SsedPartialIndexScanCursor {
                    component_index: 8,
                    page_index: 121,
