...
- `/tmp/lvcore-all-corpora-validation-20260614-partial-prefix-native-proof-v1.jsonl`
- Produced after the current scoped LVCore SSED changes:
  ...
  - initial backward native pages on large SSED indexes run the same one-leaf
    proof for specific limit-1 non-pure-letter queries, emitting a normal offset
    cursor when the next suffix hit is already visible while keeping exact mode,
    pure ASCII-letter queries, and ASCII punctuation cases deferred;
  - initial partial-prefix pages on large SSED indexes prove specific limit-1
    non-pure-letter native prefix continuations before wrapping them as
    `ssed-partial-prefix:*`, emitting probeable `ssed-partial-prefix:1` cursors
    when the next prefix hit is visible;
  ...
  - full-text non-prefix title continuation proof now verifies fast mixed
    ...
    deferred; and
  - title-label fallback pages on tiny SSED index sets use an exhaustion-level
    hit-page budget for initial exact searches, proving cheap no-continuation
  ...
  - exact title-label fallback pages with a very early first hit in
    ...
    to prove duplicate-target exhaustion or a verified next distinct hit; and
  - title-label fallback continuation cursors now carry seen body targets and
    advance by bounded scan chunks as `ssed-title-label-seen:*`, avoiding
    duplicate continuations and removing the title-label unverified cursor
    bucket.
- 336 reconstructed package paths validated with package status 336 `ok`.
- Format-family coverage: 1 `hourei`, 23 `lvl_multi_view`, 87 `lved_sqlite3`,
  and 225 `ssed` packages.
- The gate was run over the documented corpus roots plus explicit
  `Other/Android` and `Other/iOS` roots.
- Warning diagnostics remain only the explicitly deferred HC common HTML
  fallback, 261 warning entries in this gate.
- `sidecar-title-unverified-row:*` direct/nested search rows dropped from 22 to
  0. The latest gate has 76 `sidecar-title-row:*` rows with successful cursor
  probes.
...
- `ssed-partial-nonprefix-unverified-index:*` rows dropped from 22 in the
  previous full gate to 0. The latest gate has 30
...
- `_DCT_GKBUSINE` `search_forward` `10` is 86 ms and now returns
  `ssed-title-label-seen:*` with cursor probe `ok` at 4 ms.
- Tiny-index exact title-label rows now use bounded seen-target continuations or
  prove cheap exhaustion:
  `_DCT_GKGOGEN` exact `〔様様〕あからさま`, `_DCT_GKTRAVEL` exact
  `1000どるとくれじっとかーどが入っていました`, `_DCT_HKBYOIN4` exact
  `グリコアルブミン`, `_DCT_IKUIKU` exact `10代の場合`, `_DCT_VIKU1000`
  exact `あきくさをごつたにつかね供へけり`, and `_DCT_YUCOGPSY` exact
  `1970年以前[1-1]認知心理学の歴史` now return no cursor; `_DCT_HKEBMBOK`
  exact `後天性免疫不全症候群` now returns `ssed-title-label-seen:*` with
  cursor probe `ok`; and `_DCT_KQJEXPRS` exact `ｂｅｓｔ` now returns no cursor.
- Early-hit exact title-label rows now prove duplicate-target exhaustion or a
  ...
  `KQJCOLLO` exact `あいくるしい` now return `ssed-title-label-seen:*`
  cursors with successful cursor probes.
...
- `_DCT_IWKOKUG8` `search_forward` `さん` returns a verified
  ...
  is also `ok` at 4 ms.
- `_DCT_NCOMP4` `search_full_text` `1計` is 443 ms with the
...
- Specific limit-1 partial-prefix rows now have 90 probeable
  `ssed-partial-prefix:*` cursors in the latest gate; 32 targeted rows moved
  from nested `ssed-offset-unverified:1` to `ssed-partial-prefix:1`.
- Deferred cursor probes in the latest gate are currently concentrated in:
  132 native offset continuations, 125 body full-text continuations, and
...
- Focused real-package validation passed:
  ...
  - `_DCT_KQNEWEJ6` backward `一覧` now returns `ssed-title-label:1`; its
    ...
    lookahead.
- Follow-up target gap from the latest full-corpus JSONL: the remaining
  title-label rows were not semantic misses; they were filled-page pagination
  gaps where the scan had already skipped duplicate body targets but the cursor
  could not carry that seen-target state into the next page.
- Code change: title-label cursors now support `ssed-title-label-seen:*`
  payloads containing the row offset plus seen body target keys. Filled pages
  advance by bounded scan chunks instead of emitting
  `ssed-title-label-unverified:*`, and continuations skip duplicate targets
  already returned on earlier pages. Legacy `ssed-title-label:*` and
  `ssed-title-label-unverified:*` cursors still decode.
- Focused tests passed:
  - `cargo fmt --check`
  - `cargo test -p lvcore title_label -- --nocapture`
  - `cargo build -p lvcore-cli`
- Focused real-package validation passed:
  - `/tmp/lvcore-focused-validate-title-label-seen-cursors-v2.jsonl`
  - 17 affected packages validated with package status 17 `ok`.
  - No `ssed-title-label-unverified:*` cursors remained in that focused set,
    including nested cursor-probe continuations.
  - `_DCT_HKEBMBOK` exact `後天性免疫不全症候群`, iOS `KQCOLEXP` exact
    `ああいえばこういう`, and iOS `KQJCOLLO` exact `あいくるしい` now use
    bounded `ssed-title-label-seen:*` continuations through cursor probes.
...
- Eighth target gap from the latest full-corpus JSONL: several exact
  title-label fallback rows were still marked `ssed-title-label-unverified:*`
  even though direct cursor probes showed tiny SSED index sets could prove
  exhaustion or a verified next row cheaply.
- Code change: initial exact title-label fallback pages now use an
  exhaustion-level hit-page budget only when the parsed SSED catalog has at
  most 256 readable supported index blocks. Cursor pages, non-exact modes,
  multi-hit pages, circle-marker queries, and larger index sets keep the
  existing 256-row hit-page budget.
- Focused tests passed:
  - `cargo fmt --check`
  - `cargo test -p lvcore title_label -- --nocapture`
  - `cargo build -p lvcore-cli`
- Focused real-package validation passed:
  - `/tmp/lvcore-focused-validate-title-label-tiny-index-v2.jsonl`
  - 26 affected/control packages validated with package status 26 `ok`.
  - Top-level `ssed-title-label-unverified:*` rows in that focused set dropped
    to 19; the full-gate direct/nested not-probed title-label reason count
    dropped from 27 to 20.
  - `_DCT_GKGOGEN` exact `〔様様〕あからさま`, `_DCT_GKTRAVEL` exact
    `1000どるとくれじっとかーどが入っていました`, `_DCT_HKBYOIN4` exact
    `グリコアルブミン`, `_DCT_IKUIKU` exact `10代の場合`, `_DCT_VIKU1000`
    exact `あきくさをごつたにつかね供へけり`, and `_DCT_YUCOGPSY` exact
    `1970年以前[1-1]認知心理学の歴史` now prove exhaustion and return no
    cursor in the full gate.
  - `_DCT_HKEBMBOK` exact `後天性免疫不全症候群` now returns
    `ssed-title-label:29` with cursor probe `ok` at 8 ms in the full gate.
  - Larger controls remain intentionally deferred: `_DCT_IWKOKUG8` exact
    `さんディー`, `_DCT_KQNEWEJ6` exact `画像一覧`, `_DCT_KQDENTAL` exact
    `0.01規定の`, and `_DCT_YHOUGO5` exact `アカウンタビリティー` still return
...
- Full-corpus regression gate passed:
  ...
  - Deferred title-label fallback continuation probes dropped from 27 to 20;
    native offset/body full-text/sidecar-title/full-text non-prefix title
    deferral counts stayed at 209/124/22/1.
- Ninth target gap from the latest full-corpus JSONL: `_DCT_KQJEXPRS` exact
  `ｂｅｓｔ` still returned `ssed-title-label-unverified:1` even though the
  package has a tiny SSED title/index set and direct cursor probing proved
  exhaustion quickly.
- Code change: the tiny-index exact title-label continuation proof no longer
  requires the query to be authoritative for dense sidecar SQL prefiltering.
  It still requires the existing initial exact title-label prepass guard, a
  single-hit page, a tiny readable index set, and no native circle marker.
- Focused tests passed:
  - `cargo fmt --check`
  - `cargo test -p lvcore title_label -- --nocapture`
  - `cargo build -p lvcore-cli`
- Focused real-package validation passed:
  - `/tmp/lvcore-focused-validate-title-label-fullwidth-latin-v1.jsonl`
  - 19 affected/control packages validated with package status 19 `ok`.
  - Top-level `ssed-title-label-unverified:*` rows in that focused set dropped
    from 19 to 18.
  - `_DCT_KQJEXPRS` exact `ｂｅｓｔ` now returns one hit in 24 ms with no
    continuation cursor.
  - Larger and broader controls remain intentionally deferred: `_DCT_MEIKYOU`
    exact `どう`, `_DCT_ARCHSIC3` exact `すがる-をとめ`, `_DCT_KOJIEN7`
    exact `◯に滅せずんば炎炎を若何いかんせん`, and `_DCT_KQNEWEJ6` exact
    `画像一覧` still return `ssed-title-label-unverified:1`.
- Full-corpus regression gate passed:
  ...
  - Format-family coverage is unchanged: 1 `hourei`, 23 `lvl_multi_view`,
    87 `lved_sqlite3`, and 224 `ssed` packages.
  - `_DCT_KQJEXPRS` exact `ｂｅｓｔ` is 23 ms in the full gate, returns one
    hit, has no continuation cursor, and records
...
- Current native-offset follow-up from the latest full-corpus JSONL: 41 SSED
  exact/backward specific limit-1 rows returned `ssed-offset-unverified:1`, so
  validation skipped the next page even when a nearby suffix hit could be proven
  within one native leaf page.
- Code change: initial native offset deferral now also runs the one-leaf
  next-hit proof for limit-1 backward queries when the trimmed query is at least
  two characters and is either non-ASCII or ASCII alphanumeric. Exact-mode
  limit-1 proof remains deferred after focused validation showed `_DCT_GEN2002`
  exact `01` rising from 8 ms to 448 ms. Pure ASCII-letter queries and ASCII
  punctuation cases such as `_DCT_GEN2004` backward `E.` remain deferred.
- Focused tests passed:
  - `cargo fmt --check`
  - `cargo test -p lvcore ssed_native -- --nocapture`
  - `cargo build -p lvcore-cli`
- Focused real-package validation passed:
  - `/tmp/lvcore-focused-validate-native-limit1-backward-proof-v4.jsonl`
  - 38 affected/control packages validated with package status 38 `ok`.
  - Targeted exact/backward `ssed-offset-unverified:1` rows dropped from 41 to
    17 in that focused set; 24 rows now return normal cursor `1` with cursor
    probe `ok`.
  - Median first-page latency delta across the targeted rows was about 15 ms.
    The largest targeted first-page latency was `_DCT_25IGAKU` backward `ッジ`
    at 177 ms, up from 73 ms.
- Final full-corpus gate passed:
  - `/tmp/lvcore-all-corpora-validation-20260614-native-limit1-backward-proof-v1.jsonl`
  - 336 packages validated with package status 336 `ok`.
  - Native offset `not_probed` rows dropped from 188 to 164. Backward native
    offset `not_probed` rows dropped from 65 to 41; exact/forward/partial native
    deferrals stayed intentionally deferred.
  - Across the 41 targeted rows, 24 now return normal cursor `1` with cursor
    probe `ok`; 17 remain `ssed-offset-unverified:1`.
- Current partial-prefix native-offset target gap from the latest full-corpus
  JSONL: 68 SSED partial rows returned nested
  `ssed-partial-prefix:ssed-offset-unverified:1`; 38 specific limit-1 rows were
  eligible for a bounded next-hit proof while pure ASCII-letter and one-character
  broad prefixes stayed deferred.
- Code change: partial-prefix page completion now detects a raw
  `ssed-offset-unverified:*` native prefix cursor before wrapping it, then runs a
  one-leaf forward native proof only for initial partial queries with limit 1
  and a specific non-pure-letter query. If the next native prefix hit is visible,
  the page emits `ssed-partial-prefix:1`; otherwise it keeps the nested
  unverified cursor.
- Focused tests passed:
  - `cargo fmt --check`
  - `cargo test -p lvcore ssed_native -- --nocapture`
  - `cargo test -p lvcore ssed_partial_prefix_proves_specific_native_offset_cursor -- --nocapture`
  - `cargo test -p lvcore partial_prefix -- --nocapture`
  - `cargo build -p lvcore-cli`
- Focused real-package validation passed:
  - `/tmp/lvcore-focused-validate-partial-prefix-native-proof-v1.jsonl`
  - 38 affected/control packages validated with package status 38 `ok`.
  - Targeted partial `ssed-partial-prefix:ssed-offset-unverified:1` rows dropped
    from 38 to 6 in that focused set; 32 rows now return
    `ssed-partial-prefix:1` with cursor probe `ok`.
  - Median first-page latency delta across the targeted rows was about 27 ms.
    The largest targeted first-page latency was `_DCT_EJJE100` partial `相遮`
    at 197 ms in focused validation.
- Final full-corpus gate passed:
  - `/tmp/lvcore-all-corpora-validation-20260614-partial-prefix-native-proof-v1.jsonl`
  - 336 packages validated with package status 336 `ok`.
  - Native offset `not_probed` rows dropped from 164 to 132. Partial native
    offset `not_probed` rows dropped from 68 to 36; exact/forward/backward native
    deferrals stayed intentionally deferred.
  - Across the 38 targeted rows, 32 now return `ssed-partial-prefix:1` with
    cursor probe `ok`; 6 remain
    `ssed-partial-prefix:ssed-offset-unverified:1`.
  - Median first-page latency delta across the targeted rows was about 27 ms.
    The largest targeted first-page latency was `_DCT_EJJE100` partial `相遮`
    at 211 ms in the full gate.

Previous full-corpus gates:

- `/tmp/lvcore-all-corpora-validation-20260614-native-limit1-backward-proof-v1.jsonl`
- Produced after adding bounded one-leaf native offset proof for specific
  limit-1 backward queries.
- `/tmp/lvcore-all-corpora-validation-20260614-native-offset-limit3-proof-v1.jsonl`
- Produced after adding bounded one-leaf native offset proof for non-pure-ASCII
  exact/forward/backward queries with limit 3 or higher.
- `/tmp/lvcore-all-corpora-validation-20260614-sidecar-title-physical-cursors-v1.jsonl`
...
| --- | ---: | --- |
| `hc_render_common_html_fallback` | 955 | Deferred HC visual rendering |
...
| `sidecar-title-row:*` cursor probed `ok` | 76 | Sidecar title physical row continuation verified |
...
| `ssed-offset-unverified:*` direct/nested cursor `not_probed` | 132 | Native offset next-page proof intentionally deferred |
...
| `ssed-partial-prefix:*` cursor probed `ok` | 90 | Partial prefix continuation pages remain probeable |
...
| `gaiji_formatting_helper_candidate` | 18 | Observed helper code candidates |
...
- `_DCT_NCOMP4`, `search_full_text` query `1計`: 443 ms, improved from 545 ms
...
- All 336 reconstructed packages in the latest baseline open and deep-validate
